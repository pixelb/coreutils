...
case $host_os in
  ...
  *)
    ...
    env -i --from0=base "$abs_top_builddir/src/env$EXEEXT" \
      --from0=merge -0 >all || fail=1
    ...
    LC_ALL=C sort -z exp >exp-sorted || framework_failure_
    env -i --from0=base "$abs_top_builddir/src/env$EXEEXT" \
      --from0=merge -u A -0 C=operand D=new >all || fail=1
    LC_ALL=C sort -z all >out || framework_failure_
    compare exp-sorted out || fail=1
    ...
    env -i --from0=base-grow "$abs_top_builddir/src/env$EXEEXT" \
      --from0=merge-grow -0 C=three >out || fail=1
    compare exp out || fail=1
    ;;
esac
