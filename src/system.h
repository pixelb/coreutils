...
/* Output --option descriptions;
   ...
   though it can be disabled in edge cases with the HELP_NO_SGR env var.  */
#define oputs(option) oputs_ (PROGRAM_NAME, option)
static inline void
oputs_ (char const* program, char const *option)
{
  static int help_no_sgr = -1;
  if (help_no_sgr && (help_no_sgr = !!getenv ("HELP_NO_SGR")))
    {
      fputs (option, stdout);
      return;
    }
