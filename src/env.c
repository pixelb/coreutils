...
static struct option const longopts[] =
{
  ...
  {"from0", required_argument, NULL, FROM0_OPTION},
  ...
  if (streq (file, "-"))
    {
      ...
      from0_buffer = fread_file (stdin, RF_BINARY, &size);
    }
  else
    from0_buffer = read_file (file, RF_BINARY, &size);

  if (! from0_buffer)
    error (EXIT_CANCELED, errno, _("cannot read %s"), quoteaf (file));
  if (size && from0_buffer[size - 1] != '\0')
